...
class GRUModel:
    ...
    def _get_sequential_dataset(self):
        ...
        dataset_y = self.dataset[['Momentum', ]]
    ...
    def train_model(self):
        ...
        r_probs = model.predict([x_test_news['Top' + str(i)] for i in range(1, 26)] + [x_test])
        r_preds = (np.array(r_probs) > 0.5) * 1
        acc = accuracy_score(y_test, r_preds)
...
if __name__ == '__main__':
    ...
    dataset = pd.read_csv('data/combined_nasdaq_news.csv', index_col='Date')
    dataset = dataset[['Close', 'Volume', 'Change', 'Momentum'] +
    ...
    gruModel = GRUModel(corpus, dataset, max_seqlen=MAX_TEXT_SEQLEN, embed_dim=EMBEDDING_DIM)
